...
class Migration_RefundTracking extends CI_Migration
{
	...
	public function up()
	{
		...
		$this->db->query('DROP TABLE IF EXISTS ' . $this->db->dbprefix('migrate_taxes'));
		$this->db->query('DROP TABLE IF EXISTS ' . $this->db->dbprefix('migrate_sales'));
		$this->db->query('DROP TABLE IF EXISTS ' . $this->db->dbprefix('migrate_payments'));
		$this->db->query('DROP TABLE IF EXISTS ' . $this->db->dbprefix('migrate_refund'));

		$this->db->query('CREATE TABLE IF NOT EXISTS ' . $this->db->dbprefix('migrate_taxes') .
			' (INDEX(sale_id))
			(
		...
		$this->db->query('CREATE TABLE IF NOT EXISTS ' . $this->db->dbprefix('migrate_sales') .
			' (INDEX(sale_id))
			(
		...
		$this->db->query('CREATE TABLE IF NOT EXISTS ' . $this->db->dbprefix('migrate_payments') .
			' (INDEX(sale_id))
			(
